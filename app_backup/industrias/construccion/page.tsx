...
import { constructionTexts } from "/constants/industries";
  ...
  return (
    <>
      <ScrollGradientTransition>
        {/* Hero Section */}
        <HeroGradientTall
          category={constructionTexts.hero.category}
          title={constructionTexts.hero.title}
          description={constructionTexts.hero.description}
          variant="yellow"
        />
              ...
              <div className="flex flex-col gap-8 max-w-[1000px] mx-auto items-center text-center">
                ...
                <div className="flex flex-col gap-12 items-center">
                  <p className="text-2xl md:text-4xl lg:text-[40px] leading-tight font-normal text-black">
                    {constructionTexts.intro.description}
                  </p>
                  <div>
                    <Button className="bg-transparent border-[1.5px] border-black text-black px-8 py-6 rounded-full text-xl relative overflow-hidden transition-all duration-500 before:absolute before:inset-0 before:bg-[linear-gradient(150deg,#FFF700_0%,#FFF700_32%,#FFE300_70%,#5dd3c0_100%)] before:opacity-0 before:transition-opacity before:duration-500 before:rounded-full hover:before:opacity-100 hover:border-0">
                      <span className="relative z-10">{constructionTexts.intro.button}</span>
                    </Button>
                  </div>
                </div>
              </div>
      ...
      {/* Solutions Section */}
      <InteractiveList items={constructionTexts.solutions.items} title={constructionTexts.solutions.title} />

      {/* Benefits Section */}
      <Section className="bg-white py-20">
        <Container>
          <BenefitsSection
            title={constructionTexts.benefits.title}
            image="assets/industrias/4_contruccion.jpeg"
            imageAlt="Benefits"
            benefits={constructionTexts.benefits.items}
          />
        </Container>
      </Section>
      ...
      <SolutionsShowcase
        label={constructionTexts.useCases.label}
        title={constructionTexts.useCases.title}
        solutions={constructionTexts.useCases.items}
      ...
      {/* Results Section */}
      <ResultsGrid
        label={constructionTexts.results.label}
        title={constructionTexts.results.title}
        items={constructionTexts.results.items}
      />

      {/* Certifications Section */}
      <Certificates description={constructionTexts.certifications.description} />
    </>
  );
}