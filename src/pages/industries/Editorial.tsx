...
import { editorialTexts } from "../../constants/industries";
  ...
  return (
    <>
      <ScrollGradientTransition>
        {/* Hero Section */}
        <HeroGradientTall
          category={editorialTexts.hero.category}
          title={editorialTexts.hero.title}
          description={editorialTexts.hero.description}
          variant="yellow"
        />
              ...
              <div className="flex flex-col gap-8 max-w-[1000px] mx-auto items-center text-center">
                ...
                <div className="flex flex-col gap-12 items-center">
                  <p className="text-2xl md:text-4xl lg:text-[40px] leading-tight font-normal text-black">
                    {editorialTexts.intro.description}
                  </p>
                  <div>
                    <Button className="bg-transparent border-[1.5px] border-black text-black px-8 py-6 rounded-full text-xl relative overflow-hidden transition-all duration-500 before:absolute before:inset-0 before:bg-[linear-gradient(150deg,#FFF700_0%,#FFF700_32%,#FFE300_70%,#5dd3c0_100%)] before:opacity-0 before:transition-opacity before:duration-500 before:rounded-full hover:before:opacity-100 hover:border-0">
                      <span className="relative z-10">{editorialTexts.intro.button}</span>
                    </Button>
                  </div>
                </div>
              </div>
      ...
      {/* Solutions Section */}
      <InteractiveList items={editorialTexts.solutions.items} title={editorialTexts.solutions.title} />

      {/* Benefits Section */}
      <Section className="bg-white py-20">
        <Container>
          <BenefitsSection
            title={editorialTexts.benefits.title}
            image="assets/industrias/6_editorial.jpeg"
            imageAlt="Benefits"
            benefits={editorialTexts.benefits.items}
          />
        </Container>
      </Section>
      ...
      <SolutionsShowcase
        label={editorialTexts.useCases.label}
        title={editorialTexts.useCases.title}
        solutions={editorialTexts.useCases.items}
      ...
      {/* Results Section */}
      <ResultsGrid
        label={editorialTexts.results.label}
        title={editorialTexts.results.title}
        items={editorialTexts.results.items}
      />

      {/* Certifications Section */}
      <Certificates description={editorialTexts.certifications.description} />
    </>
  );
}